...
def loadOptions():
    ...
    ynheadless = input(
        colored('[?] Enable Headless Mode? (recommended) [Yes/no]: ', 'yellow'))
    ...
    loaddefault = input(
        colored('[?] Load default settings? (recommended) [Yes/no]: ', 'yellow'))
    ...
    for opt, dval in optionset.items():
        if loadall:
            ...
            userinp = input(
                colored(f"[?] {dval['q']} [Yes/no]: ", "yellow")).strip().lower()
...
def combineImages(image_prefix):
    images = []
    image_files = [f for f in os.listdir(os.getcwd()) if (f.startswith(
        image_prefix) and f.endswith('.png')) and not ('_final' in f)]
    ...
    try:
        ...
        print(colored(
            f"[-] Unable to create new image : Stopping the combining of images with prefix '{image_file}' : {e}", "red"))
        return
    ...
    for class_name in classes:
        try:
            driver.execute_script(
                f"document.querySelector('.{class_name}').remove();")

            # I DONT KNOW :-(
            if class_name == "fuse-h-280.mb-5":
                driver.execute_script(
                    f"document.querySelector('.{class_name}')[0].remove();")
                driver.execute_script(
                    f"document.querySelector('.{class_name}')[1].remove();")

            print(colored(f"[+] Removed element with  .{class_name}", "green"))
        except Exception as e:
            print(colored(f"[-]: Error removing '.{class_name}': {e}", "red"))

    current_page_title = driver.find_element(
        by=By.CLASS_NAME, value="resource-title").text
    print(colored(f"[+] Resource Title: {current_page_title}", "green"))

    print(
        colored(f"[+] Setting window resolution to {width}x{page_height}", "green"))
    driver.set_window_size(width, page_height)

    windowinnerHeight = driver.execute_script("return window.innerHeight")

    scroll_how_many_times = math.floor(
        int(page_height) / int(windowinnerHeight))
...
def run():
    ...
    ynmoreurl = input(
        colored("[?] Load Sub URLs? [Yes/no]: ", "yellow")).strip().lower()
    urls = loadUrls()
    final_urls = set()
    ...
    options = loadOptions()
    yndel = input(
        colored("[?] Delete original files? [yes/No]: ", "yellow")).strip().lower()
    deleteimg = True
